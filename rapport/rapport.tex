...
Cependant, cet objectif peut être divisé en plusieurs sous-objectifs.\\
...
Un voisinage est un des meilleurs exemples de mixité sociale.
...
Le moyen pour la vérification compétences des n'est pas encore définie avec exactitude mais comme solutions envisagés il y a la certification par diplômes et/ou la certification de compétence par les usagers.
Si X personnes assure que la compétence utilisé lors du service rendu est acquise, la certification peut être donnée.
...
\begin{itemize}
  \item AlloVoisins : propose un service de mise en relation entre particuliers avec 3,5 millions
...
Art Entraide se différencie par une certification plus poussée.
...
\subsection{Différents types de comptes}
...
Étant donné que ces comptes certificateurs auront la capacité de certifier les autres comptes (une responsabilité importante), une authentification
plus poussée devra être mise en place pour s'y connecter.\\
...
modèle relationnel sont plus indiqués. Compte tenu du temps imparti et de nos connaissances,
...
SSO aux réseaux sociaux (social login) comme google ou facebook qui permettraient aux personnes âgés de ne
pas avoir à s’inquiéter de retenir un mot de passe et ainsi faciliter l’utilisation de
notre sites.  En raison de l’échéance, l’authentification se fera de façons basique à travers un simple
identifiant(l'email) et mot de passe sur HTTPS. Les informations d’identification seront stockées dans une
partie spécifique de la base de données. Cette dernière stockera aussi les préférences des utilisateurs.
Si le temps le permet on ajoutera le social login avec google.\\

Le tableau ci dessous résume les possibilitées pour chaque catégorie et montre (cases bleus)
...
Compte tenu des choix techniques fait pour la réalisation (HTML/CSS, nginx, PHP, PostgreSQL) nous avons
...
Les diagrammes de sequence de haut niveau permettent de mieux définir la dynamique des cas d'utilisations dans le cadre de notre
modèle MVC. Nous présentons en particulier ci-dessous les diagrammes correspondant à l'authentification et à la
création d'une annonce. \\

\begin{figure}[H]
  \includegraphics[width=\linewidth]{../Conception/DS_Connexion.png}
  \caption{Diagramme de Séquence haut niveau représentant la connexion}
  \label{fig:<un-label-court>}
\end{figure}
...
Notamment pour la cohérence interne: Le haut et bas de page sont communs à toutes
les pages du site.
...
Si l'utilisateur est déjà connecté, les boutons "Se connecter" est "S'inscrire"
sont remplacés par le nom de la personne avec sa photo de profil, qui seront
...
du posteur de l'annonce témoignant de sa certification s'il en a une. La catégorie d'une annonce
...
\subsubsection{Page de recherche d'annonce}

Sur cette page, les critères de recherches sont tous groupés sur une mème ligne
Et les previews des annonces ont été reprises de la page d'accueil pour plus de
cohérence interne.
