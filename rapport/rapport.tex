...
\begin{figure}[H]
  \includegraphics[width=200px]{../Conception/PHP/DC.png}
...
La classe Utilisateur contient les informations des utilisateurs. Un utilisateur peut crée des annonces, des messages et accéder aux réponses qui le concernent (soit d'une de ses annonces, soit d'une annonce à laquelle il a répondu).
Une annonce appartient à une categorie.

Pour la même raison de lisibilité, les \textit{getters} et \textit{setters} n'apparaissent pas, mais il y en a un pour chaque attribut.
...
\begin{itemize}
    ...
    \item Des informations personnelles telles que le nom, prénom, adresse qui servent à reconnaître un utilisateur.
    \item Un booléen "certif" servant à savoir si l'utilisateur est certifié ou non.
    \item Une réputation, qui pour l'instant n'est pas exploitée, mais servira à terme à faciliter la confiance entre les utilisateurs.
\end{itemize}

Un certificateur est un utilisateur qui ne peut pas créer d'annonce et ne possède pas d'attribut adresse, certif, et réputation.\\
Une annonce possède :
\begin{itemize}
    \item Un nom et une description qui servent à caractériser la demande de l'utilisateur
    \item Une adresse et une date de service qui sert à connaître le lieu du service et la potentielle date désirée.
...
Un message possède :
\begin{itemize}
    \item Une chaîne de charactères contenant le corps du message.
...
aussi tous les outils nécessaires au développement (éditeur de texte + it). Ainsi les développeurs
...
Pour la deuxième itération, le projet a été découpé en 3 parties. L’équipe a été divisée en 3 binômes.\\
...
Sur ce diagramme, Alexandre et Lucas se sont occupés du visuel du site,
Elian et Marion de la logique de fonctionnement et
Eliott et Vincenzo du stockage des données.\\
...
Pendant la troisième itération, les équipes ont pu terminer leurs tâches, puis toute l'équipe a été de nouveau regroupée
pour s'occuper des derniers détails et correction de bugs.
...
Les utilisateurs connectés peuvent créer une annonce depuis le bouton "Créer une annonce" accessible
n'importe où sur le site ; on leur demande alors de rentrer les informations relatives à leur annonce
(titre, description, etc...) et leur annonce est enregistrée dans la base de données.
Ces utilisateurs peuvent ensuite accéder aux annonces qu'ils ont déja créées depuis la page de leur profil.\\

Une page "Rechercher une annonce" affiche les annonces, récupérées dynamiquement depuis la base de données,
sous forme d'aperçus (titre, catégorie, début de description) ; elle permet également de les filtrer par catégorie,
ou de rechercher par mots-clés (pour l'instant assez sommaire).\\

Chaque aperçu d'annonce permet d'accéder à une page présentant plus en détails l'annonce en question.\\
Depuis cette page, le créateur de l'annonce peut la modifier ou la supprimer ; les autres utilisateurs peuvent
répondre à l'annonce (voir section "Messages") ou la signaler si elle est inapropriée.\\
...
Cependant, en ce qui concerne l'adaptabilité, nous avons décidé de ne pas en tenir compte et d'éviter une surcharge de paramètres et options pour garder une simplicité optimale. Nous n'avons pas non plus tenu compte de la compatibilité,
...
\begin{enumerate}
  ...
  \item Tenter d'y répondre (ce qui est impossible sans être connecté)
  \item Créer un compte
  \item Répondre à l'annonce recherchée plus tôt
  \item Créer une annonce
  \item La modifier
  \item Modifier son compte (créé plus tôt)
...
Il reste quelques problèmes qui n'ont pas été corrigés -- bien que détectés -- au vu du temps restant,
...
\begin{enumerate}
  ...
  \item Si le détail de l'annonce est trop conséquent, test réalisé avec un texte de 2 Mo copié dans le champ Description, la page ne répond plus. il faut une fermeture/rechargement de l'accueil
...
\begin{figure}[H]
  \includegraphics[width=400px]{../Conception/DS_Connexion.png}
...
\begin{center}
   \begin{tabular}{ | l | c | c | c | c | c | c | }
     ...
     Maryvonne Prely    & 9.5/10  & 10/10      & 9/10   & 10/10   & 10/10      & 10/10   \\ \hline
     Xavier Sirvent     & 9.25/10 & 9/10       & 9/10   & 9.5/10  & 10/10      & 9.6/10   \\ \hline
     Laure Chanas       & 10/10   & 10/10      & 10/10  & 9/10    & 10/10      & 9/10    \\ \hline
     Total              & 8.78/10 & 8.7/10    & 8.8/10  & 9.4/10  & 9.4/10     & 9.4/10 \\ \hline
